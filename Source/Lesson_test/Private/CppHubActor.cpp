...
void ACppHubActor::OnTimeToSpawn(){
	if (++CurrentTimerCount <= TotalTimerCount) {
		...
		ACppBase* SpawnObject = GetWorld()->SpawnActor<ACppBase>(CppClass, Location, Rotation);
		if (SpawnObject) SpawnedObjects.Add(SpawnObject);
	}
	else {
		GetWorldTimerManager().ClearTimer(SpawnTimerHandle);
		for (int i = TotalTimerCount; i >= i; i--) {
